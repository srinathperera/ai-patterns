...
class Config:
    ...
    community_root_dir: Path = Path(root, "outputs",DEFAULT_TAG,VERSION,"callgraph_communities")
    # Where to write the embeddings CSV
    output_path: Path = Path(root, "outputs",DEFAULT_TAG,VERSION,"callgraph_embeddings.csv")
...
def iter_pattern_files(community_root_dir: Path) -> List[tuple[str, Path]]:
    repo_dirs = [p for p in sorted(community_root_dir.iterdir()) if p.is_dir()]
    if not repo_dirs:
        raise ValueError(f"No repositories detected in {community_root_dir}")
...
def build_embeddings_dataframe(config: Config) -> pd.DataFrame:
    files = iter_pattern_files(config.community_root_dir)
    ...
    for start_idx in range(0, total_files, config.batch_size):
        ...
        for (repo_name, py_path), embedding_vector in zip(batch, embeddings):
            records.append(
                {
                    "repo": repo_name,
                    "file": str(py_path.relative_to(config.community_root_dir)),
    ...
    for record in records:
        row = {f"emb_{i+1}": value for i, value in enumerate(record["embedding"])}
...
def main(config: Config = Config()) -> None:
    if not config.community_root_dir.exists():
        raise FileNotFoundError(f"Root directory not found: {config.community_root_dir}")
